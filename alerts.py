...
class HTTPPostAlerter(Alerter, PrometheusWrapper):
    ...
    def __init__(self, rule):
        ...
        self.timeout = self.rule.get('http_post_timeout', 120)
    ...
    def alert(self, matches):
        ...
        for match in matches:
            ...
            apiVersion = "apis/functions.tatacommunications.com/v1alpha3"
            eventType = "com.tatacommunications.esrule.matched"
            ...
            for url in self.post_url:
                try:
                    ...
                    resourcegroup = "estriggers.functions.tatacommunications.com"
                    rule = ESTriggerName + '_' + ESTriggerNamespace

                    headers = {
                        "datacontenttype": "application/json",
                        "type": eventType,
                        "source": source,
                        "subject": apiVersion + "/namespaces/" + ESTriggerNamespace + "/estriggers/" + ESTriggerName,
                        "Kind": kind,
                        "Name": ESTriggerName,
                        "Namespace": ESTriggerNamespace,
                    }
                    ...
                    with zipkin_span(
                        service_name='',
                        span_name='estrigger-controller',
                        ...
                        #do_stuff()
                        time.sleep(10)
                    elastalert_logger.info('sending cloudevent id: %s, source: %s, subject: %s, target: %s' % (
                    event['id'], event['source'], event['subject'], target))


                    response = requests.post(url, data=ce_body,
                                             headers=ce_headers, proxies=proxies, timeout=self.timeout)
                    ...
                    PrometheusWrapper.prom_event_count.labels(rule_name=rule, event_source=source,
                                                              event_subject=apiVersion + "/namespaces/" + ESTriggerNamespace + "/estriggers/" + ESTriggerName,
                                                              event_type=eventType, esTrigger_name=ESTriggerName,
                                                              esTrigger_namespace=ESTriggerNamespace,
                                                              resource_group=resourcegroup, sink=target,
                                                              response_code=statuscode,
                                                              response_code_class=str(statusclass) + 'xx').inc()
                    raise EAException('failed to send cloudevent: %s, id: %s, source: %s, subject: %s, target: %s' % (
                    e, event['id'], event['source'], event['subject'], target))
            elastalert_logger.info('cloudevent sent id: %s, source: %s, subject: %s, target: %s' % (
            event['id'], event['source'], event['subject'], target))
